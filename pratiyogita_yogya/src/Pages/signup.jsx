...
export default function Signup() {
  ...
  async function handleGoogleLogin() {
    try {
      ...
      setError('Failed to sign up with Google: ' + getFirebaseAuthErrorMessage(err));
    }
    setLoading(false);
  }
  ...
  return (
    <Box sx={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', bgcolor: '#2b1e17' }}>
      <Paper elevation={3} sx={{ p: 4, width: '100%', maxWidth: 400, bgcolor: '#3d2419', color: '#FBF6EE' }}>
        <Typography variant="h4" mb={3} textAlign="center" color="#E4572E">Sign Up</Typography>
        {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
        <form onSubmit={handleSubmit}>
          <TextField 
            fullWidth label="Full Name" type="text" value={name} onChange={e => setName(e.target.value)}
          ...
          />
          <TextField 
            fullWidth label="Confirm Password" type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
        ...
        <Typography mt={2} textAlign="center" sx={{ color: 'rgba(232,216,195,0.7)' }}>
          Already have an account? <Link to="/login" style={{ color: '#E4572E', textDecoration: 'none', fontWeight: 'bold' }}>Log In</Link>
        </Typography>
      </Paper>
    </Box>
  );
