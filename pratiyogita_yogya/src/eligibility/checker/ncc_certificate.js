 ...
 * {
 *   "ARMY WING": ["C CERTIFICATE"],
 *   "NAVAL WING": ["C CERTIFICATE"],
...
const isObjectFormat = (value) => {
    ...
    return keys.some(k => k.includes('ARMY') || k.includes('NAVY') || k.includes('NAVAL') || k.includes('AIR'));
};
...
const getAllowedCertificatesForWing = (nccCertObj, userNccWing) => {
    const normalizedWing = normalizeKeyLoose(canonicalizeWingValue(userNccWing));
    
    for (const [key, value] of Object.entries(nccCertObj)) {
        if (normalizeKeyLoose(canonicalizeWingValue(key)) === normalizedWing) {
