...
/**
 * Resolve caste-wise structures across age and DOB fields.
 *
 * Supported data shapes:
 * 1. Flat/session-wise: { "2026": "...", "2027": "..." } or scalar string values
 *    -> Returns as-is
 * 2. Caste-wise: { "GEN": "..." } or { "GEN": { "2026": "..." }, ... }
 *    -> Returns the value/sub-object for user's caste category
 */
export const resolveCasteWiseValue = (valueData, userCategory = null) => {
    if (!valueData || typeof valueData !== 'object' || Array.isArray(valueData)) return valueData;

    const keys = Object.keys(valueData);
    if (keys.length === 0) return valueData;

    const isCasteWise = keys.some((key) => CASTE_KEYS.includes((key || '').toUpperCase()));
    if (!isCasteWise) {
        return valueData;
    }
    ...
    const exactCategoryKey = keys.find((key) => key.toUpperCase() === category);
    if (exactCategoryKey) {
        return valueData[exactCategoryKey];
    }

    const genKey = keys.find((key) => key.toUpperCase() === 'GEN');
    if (genKey) {
        return valueData[genKey];
    }

    return valueData[keys[0]];
};

/**
 * Backward-compatible alias for between_dob specific call sites.
 */
export const resolveCasteWiseDob = (betweenDobData, userCategory = null) =>
    resolveCasteWiseValue(betweenDobData, userCategory);
...
export const checkDateOfBirth = (userDobStr, examData, examSession = null, examCode = null, userCategory = null) => {
    ...
    switch (criteriaType) {
        ...
        case 'STARTING_AGE':
        case 'MIN_AGE': {
            const resolvedStartingAge = resolveCasteWiseValue(examData.starting_age, userCategory);
            const ageValue = getValueForSession(resolvedStartingAge, examSession, examCode);
        ...
        case 'MAX_AGE': {
            ...
            const resolvedEndingAge = resolveCasteWiseValue(examData.ending_age, userCategory);
            let ageValue = getValueForSession(resolvedEndingAge, examSession, examCode);
            
            // NEET uses minimum_dob with MAX_AGE criteria type
            if (!ageValue && examData.minimum_dob) {
                const resolvedMinimumDob = resolveCasteWiseValue(examData.minimum_dob, userCategory);
                const minDobValue = getValueForSession(resolvedMinimumDob, examSession, examCode);
        ...
        case 'BETWEEN_AGE': {
            ...
            const resolvedBetweenAge = resolveCasteWiseValue(examData.between_age, userCategory);
            let betweenAgeValue = getValueForSession(resolvedBetweenAge, examSession, examCode);
        ...
        case 'MAXIMUM_DOB':
        case 'MAX_DOB': {
            const resolvedMaximumDob = resolveCasteWiseValue(examData.maximum_dob, userCategory);
            const maxDobValue = getValueForSession(resolvedMaximumDob, examSession, examCode);
        ...
        default: {
            ...
            if (examData.between_age) {
                const resolvedBetweenAge = resolveCasteWiseValue(examData.between_age, userCategory);
                const betweenAgeValue = getValueForSession(resolvedBetweenAge, examSession, examCode);
                if (betweenAgeValue) {
                    const parsed = parseAgeRange(betweenAgeValue);
                    if (parsed) {
                        const check = checkBetweenAge(userDob, parsed.minAge, parsed.maxAge, referenceDate);
            ...
            if (examData.ending_age) {
                const resolvedEndingAge = resolveCasteWiseValue(examData.ending_age, userCategory);
                const ageValue = getValueForSession(resolvedEndingAge, examSession, examCode);
                if (ageValue) {
                    const check = checkEndingAge(userDob, ageValue, referenceDate);
                    examRequirement = `Maximum age: ${ageValue} years`;
