...
export const checkFullEligibility = (userInput, examData, examSession = null) => {
    ...
    if (examEducationLevels && typeof examEducationLevels === 'object') {
        ...
        // Use exam-defined levels (only levels present in exam JSON)
        // instead of all levels from highest down
        const examDefinedLevels = getExamDefinedEducationLevels(examEducationLevels);
        // Fallback to old behavior if no exam-defined levels found
        const requiredLevels = examDefinedLevels.length > 0 
            ? examDefinedLevels 
            : getRequiredEducationLevels(examEducationLevel);
