...
/**
 * Get only the education levels that the exam actually defines in its education_levels object.
 * Used in Exam Basis mode to show only relevant education fields.
 * For example, CDS IMA only defines 'graduation' → only graduation row shown.
 * CDS INA defines 'graduation' + '12th_higher_secondary' → both shown.
 * 
 * @param {Object} examEducationLevels - The exam's education_levels object
 * @returns {string[]} Array of education level keys that exist in exam JSON, ordered by hierarchy
 */
export const getExamDefinedEducationLevels = (examEducationLevels) => {
    ...
    const examKeys = Object.keys(examEducationLevels);
    
    // Return only the levels that exist in the exam JSON, in hierarchy order
    return orderedLevels
        .filter(level => examKeys.includes(level.key))
        .map(level => level.key);
};
...
// ============================================
// FORM OPTION HELPERS (edu_final-backed)
// ============================================
...
const DEFAULT_ACTIVE_BACKLOG_VALUES = ['0', '1', '2', '3', '4', '5', '5+'];
const DEFAULT_GAP_YEAR_VALUES = ['0', '1', '2', '3', '4', '5', '>5'];

const normalizeLevelKeyForOptions = (levelKey) => {
    if (!levelKey) return null;
    return getEducationLevelKey(levelKey) || levelKey.toString().toLowerCase().trim().replace(/\s+/g, '_');
};

const normalizeRawOptionValues = (rawValue) => {
    ...
    if (typeof rawValue === 'object') {
        if (Array.isArray(rawValue.options)) {
            return rawValue.options.map((value) => String(value).trim()).filter(Boolean);
        }
        if (Array.isArray(rawValue.values)) {
            return rawValue.values.map((value) => String(value).trim()).filter(Boolean);
        }
    }
...
const createUniqueValues = (values = []) => {
    const unique = [];
    const seen = new Set();
...
const toSelectOptions = (values, emptyLabel = 'Select', labelMap = {}) => {
    const uniqueValues = createUniqueValues(values);
    return [
...
export const getActiveBacklogsOptionsForLevel = (levelKey = null) => {
    ...
    return toSelectOptions(values, 'Select', {
        '0': 'No Backlogs'
    });
};
 ...
 */
export const checkEducationEligibility = (userInput, examData) => {
    const results = [];
